...
- `routes/model-candidate-briefing.md`：逐问宽候选、文献状态、优劣势和 Leader 推荐；
- `routes/human-model-decision.md`：真实用户选择的主模型、保留候选和实现限制；
- `routes/route-handoff.md`：按人工决定冻结的主路线、备选/敏感性模型、baseline、切换与重开条件；
- `inputs/source-manifest.json`：原始材料版本、路径和哈希。

后半程负责把人工授权路线转化为可复现的数据、模型、结果、图表准备材料与论文证据，但不能静默改写题意基线或模型家族。若执行中发现目标不可构造、接口失效、路线必须制造关键标签或核心模型无法由数据支撑，应回到最早受影响阶段；改变模型家族、目标或核心结构时必须重新进入 H1。
...
1. 数据、建模、验证、图表准备、正式绘图和论文分别承担不同责任，不由一个 Agent 从头包办；正式图只消费 F4 授权数据包。
...
### 3.0 贯穿模块：文献、人的意见与引用证据

**实现状态：REF0–REF6 已实现。** 设计见 [literature-research.md](literature-research.md)，配置见 `literature-team.json`，角色 prompt 位于 `prompts/literature/`，开放模板位于 `templates/literature/`。

W5A 后，文献 Scout 与结构 critic 并行：前者逐候选检索原始方法、领域应用、限制、负面证据，并发现 Agent 未想到的替代模型；Human Recorder 形成具体咨询问题并只记录真实人类回复；独立 Auditor 防止标题/摘要过度推断和人的意见冒充证据。W5C 后 Leader 形成候选汇报，H1 由真实用户决定，才生成 L2。V6/CP1 后再次进入，形成 citation gap、source notes、`references.bib`、claim-to-citation map 和 references handoff。

Zotero 是可选只读来源，不是依赖。向 Zotero 导入、保存或修改记录需要用户明确授权；Zotero item key 与 BibTeX key 分开管理。
...
该支线按 F0 冻结、F1 逐问 Curator、F2 流式 Evidence Auditor、F2R 原 Curator 回应、F3 跨问/章节位置整合、F4 交接执行。F3 Integrator 是 `figure-plan.md` 与 `figure-preparation-handoff.md` 的唯一内容 owner；F4 Leader 只核对条件、处理回滚和宣布汇合。诊断图不能直接晋升为论文图；FR0–FR4 必须从验证授权的数据包重新绘制。数据、筛选、聚合、单位或 claim 发生改变时，必须返回上游，而不是在绘图阶段修饰掩盖。
...
```mermaid
flowchart LR
    A["题意基线与 Route A/B 宽候选"] --> R["REF0–REF3 候选文献与人的意见校准"]
    R --> HG["L2C 候选汇报 / H1 真实人工决定"]
    HG --> B["共享数据工程"]
    B --> C["按题建模与实验"]
    C --> D["独立验证"]
    C --> E["诊断产物"]
...
1. Route A/B 后并行完成结构审查、候选文献检索和真实人类咨询；W5C 重构候选，Leader 汇报后等待 H1，按真实决定形成 L2；
2. 建立共享数据基础；
...
6. 修正、切换路线或局部回滚；
7. 确认可引用的结果和证据；
8. V6 后同时启动图表准备 F0、论文准备 CP0 和 REF4–REF6；
9. CP1 先提供最小章节地图，图表 F3、逐问章节材料与正式引用补齐随后并行；
10. 完成图表交接和 CP4–CP6 双层审查后的论文框架交接；
11. F4 后 FR0–FR2 与论文准备/写作并行，FR3 在事实稳定正文中做真实版面检查，FR4 交接正式图；
12. PW0–PW7 生成正式论文 Markdown，并完成四类独立审查；
13. 正式图与已审引用齐备后，FD0–FD3 组装论文与支撑材料候选包；
14. FD4–FD7 五路终审，只报告问题并交给人最终微调和投稿。
...
正式论文绘图代码、pilot、v1/final 图、review 和 manifest 由 FR0–FR4 在 `formal-figures/` 中留档，不写回数据/建模/验证目录。
...
每问形成可供论文 Agent 使用的材料包，至少包含：方法说明、变量与公式、假设、结果表、论文候选 Figure ID、图表准备包、图注方向、稳健性、限制和来源索引。图表准备支线可以与章节材料包并行；正式论文图由 FR0–FR4 并行完成，PW 使用占位，FD0 消费 rendering handoff。
...
后半程继续使用一个 Leader，不为每个模块复制前半程六角色结构。W/D/M/V 保持既有最多三个并发 worker；V6 后图表、论文准备、正式写作逐问任务和 FD4 五路独立终审按独立写入路径并行，不设固定数字上限。
...
交接题意基线、候选模型汇报、真实人工决定、按决定形成的路线交接、每问分析单位、目标量、题间接口、数据风险和禁止默认事项。
...
### 正式论文 Markdown → 人工最终交付

FD1 将最终结果数据和实际运行脚本完整源码整理为支撑材料；FD2/FD3 只处理格式与资源绑定并冻结候选快照；FD4 从排版、扣题、语言、交付证据和全链路一致性五个方向独立终审。终审问题不再由 Agent 自动修改，而是连同候选包一起交给人。
...
1. **已实现：** 文献/真实人类意见 REF0–REF3 与正式引用 REF4–REF6、独立审查、BibTeX 和引用交接；
2. **已实现：** 数据工程工作流 D0–D5；
...
10. **人工处理：** 终审后人工微调和实际投稿；
11. **待实现：** 历史 C 题完整回放与 harness 评测。

## 12. 仍未实现的内容

文献与引用、数据、建模、验证、图表准备、正式绘图、论文准备、正式写作和最终交付之后，目前仍未实现：

- 历史 C 题完整回放与 harness 评测；
- 具体题目的模型、求解代码、结果、正式图片、排版文件和答卷，这些由实际 run 产生；
- 终审后人工微调和实际投稿。

正式论文图的绘制、审美评价和受限视觉迭代由 FR0–FR4 负责；最终交付只消费 manifest 授权资产并停止于人工接管，不替人投稿。
