...
- 文献 Scout、V6 后的图表/正式绘图/论文准备/正式写作逐问任务，以及 FD4 五路独立终审是显式并行例外：不设固定数字上限，只在输入已经冻结、写入根互不冲突且平台容量允许时并行；不得按单篇论文、单条引用、候选图、段落、小节或单个问题点制造 Agent。正式绘图新 Agent 额外强制显式 astra-high 覆盖，不继承默认 Luna。
