...
正式论文图由 FR0–FR4 的 astra-high Visual Producer 消费本支线交接包生成。正式绘图模块不得据此自行改变数据口径、筛选、聚合、单位或验证授权范围；如果需要改变这些内容，必须产生变更请求返回上游。
...
- 正式绘图 Producer 是否只凭交接包即可作图，不必重新猜测代码意图。
...
- 汇总诊断异常、已裁决 change request、仍保留的分歧和 FR0–FR4 注意事项；
- 生成全篇 `figure-plan.md`；
...
6. 交接明确不建议作图和 FR0–FR4 不得改变的口径。

结果章节可以在支线运行期间使用 Figure ID 占位，但不得在上述条件未满足时标记定稿。F4 完成后停止图表准备并显式进入正式绘图 FR0–FR4，后者负责：
...
正式绘图若发现数据语义问题，必须回到本交接包或更早上游，不得以视觉修改掩盖问题。
...
- 诊断异常、已裁决回滚、未决 change request 和 FR0–FR4 注意事项。
...
- 只是审美分歧：记录给 FR Reviewer/Leader 或人，不在 F0–F4 内评分或迭代。
...
按 [正文深度与逐步骤图量协议](protocols/paper-depth-and-visual-coverage.md)执行：研究正文默认25–35页；每个研究步骤1–3张图。CP/PW须同时审查扩充与删减，12页短稿不能自动验收；F/FR覆盖数据统计和预处理等前置步骤；FD报告真实正文页数和嵌入图数，不靠源码附录/缩放凑数。官方原文和用户目标分开记录，不能自动沿用20页上限。数据不足按原owner补证，不虚构。此变更仅用于新规划/派工，既有冻结run不自动迁移。
