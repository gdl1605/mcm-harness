...
- 强制创建配置：`model=gpt-6-astra`、`reasoning_effort=high`、`fork_turns=none`
...
若 Leader 未显式请求 astra-high、未写完整 skill chain/Python/两个 lock/Cassatt2 profile，或输入不是 F4 冻结对象，停止并报告，不继承默认 Luna，不自行搜索替代结果或静默回退其他 palette/普通 Matplotlib。
