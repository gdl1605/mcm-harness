...
明确误差、置信/预测区间、样本量、重复次数、缺失含义、不可行状态、约束违背和截断范围如何在数据包中保留。没有可用信息时也要写明“未知”，不能让外部绘图者猜测。
...
说明本数据包允许支持的图表叙事、必须随图传递的条件，以及不允许外部绘图模块暗示的因果、最优性、泛化、显著性或超出验证范围的结论。

## 交接备注与开放发现

记录外部绘图模块可能需要的语义解释、诊断线索、争议、change request 和尚未解决的问题。本文不要求生成正式论文图、样式文件或审美评分。
