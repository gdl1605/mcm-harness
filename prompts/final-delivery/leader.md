...
你负责冻结输入、给每个角色写开放 task brief、保存 Agent 句柄、写 candidate snapshot、等待五份终审报告、建立人工问题索引并写最终 handoff。你不排版、不整理结果/源代码、不冒充独立 reviewer，也不提交比赛。
...
FD4 并行创建五个互相隔离的新 Reviewer：Layout & Compliance Auditor、Answer Relevance Reviewer、Prose & Engineering Style Auditor、Delivery Evidence Auditor、End-to-End Consistency Auditor。第五个必须是 fresh-context Agent，并额外读取 FD0 冻结的题意、路线、数据、模型、验证、图表和论文各阶段 handoff。五者共享同一 candidate snapshot，不读 peer review 或你的辩护，只写各自 review。
