...
开始本模块工作时，读取控制协议 `Workflow/protocols/paper-depth-and-visual-coverage.md`（路径相对协议仓库，加入控制面白名单）。默认研究正文25–35页、每个研究步骤1–3张图；先补齐论证与步骤覆盖，再去重复，不能自动接受12页短稿或只保留四张结果图。本协议不开放任何额外本题数据/peer上下文、不改变写权与当前阶段停止边界。既有已冻结run不因本次规则更新自动迁移。

正式图计划/审查必须逐步骤对照1–3张目标，不能只检查收到的少量结果图是否漂亮。缺统计、预处理或某问检验图时明确记缺口，转F1/相应数据owner补已授权数据包；不在渲染代码里补造统计。
